...
def main():
    ...
    with Session(engine) as session:
        ...
        factures_test = session.exec(
            select(Facture).where(Facture.fournisseur_id == fournisseur_id).limit(20)
        ).all()

        if factures_test:
            facture_options = {
                f"{f.numero_facture} - {f.montant_comptable}€": f for f in factures_test
            }
            ...
            texte_test = st.text_area(
                "Texte de test manuel", height=200, placeholder="Collez ici un texte de test..."
            )
        ...
        with col_gestion:
            ...
            if regles:
                for regle in regles:
                    with st.expander(
                        f"{'✅' if regle.actif else '❌'} {regle.table_cible.value}.{regle.champ_cible}",
                        expanded=False,
                    ):
                        ...
                        with col2:
                            nouveau_actif = st.checkbox(
                                "Actif", value=regle.actif, key=f"actif_{regle.id}"
                            )
                        ...
                        if st.button("🧪 Tester cette règle", key=f"test_{regle.id}"):
                            if fournisseur_id is not None:
                                test_result = tester_regle_extraction(
                                    fournisseur_id, texte_test, nouveau_regex
                                )
                                if test_result:
                                ...
                                st.error(
                                    "❌ Impossible de tester la règle : fournisseur_id est manquant."
                                )
        ...
        with col_ajout:
            ...
            with st.form("nouvelle_regle", clear_on_submit=True):
                ...
                nouveau_regex_form = st.text_input(
                    "Regex d'extraction", placeholder="ex: numero.*?(\\d+)"
                )
                ...
                if st.form_submit_button("Ajouter la règle"):
                    if nouveau_champ and nouveau_regex_form:
                        if fournisseur_id is None:
                            st.error(
                                "❌ Impossible d'ajouter la règle : fournisseur_id est manquant."
                            )
                        else:
                            try:
        ...
        with col_test_custom:
            ...
            with st.form("test_regex_form"):
                ...
                bouton_tester = st.form_submit_button(
                    "🧪 Tester cette regex", use_container_width=True
                )
