                ...
                **Total:** {base_info['total_tantiemes']:.2f}
                ...
                st.dataframe(
                    ...
                    column_config={
                        "numero_ug": "N° UG",
                        "numero_ca": "N° CA",
                        "debut_occupation": st.column_config.DateColumn(
                            "Début occupation", format="DD/MM/YYYY"
                        ),
                        "fin_occupation": st.column_config.DateColumn(
                            "Fin occupation", format="DD/MM/YYYY"
                        ),
                ...
                with col_stat2:
                    total_tantieme = sum(
                        float(t["tantieme"]) for t in tantiemes_data if t["tantieme"]
                    )
    ...
    try:
        with Session(engine) as session:
            ...
            for base in bases:
                ...
                total_tantiemes = sum(
                    float(t.tantieme) for t in tantiemes if t.tantieme is not None
                )
...
def get_tantiemes_detail(controle_id: int, base_code: str) -> List[Dict]:
    ...
    try:
        with Session(engine) as session:
            ...
            tantiemes = session.exec(
                select(Tantieme)
                .where(Tantieme.base_repartition_id == base.id)
                .order_by(Tantieme.numero_ug)
            ).all()
