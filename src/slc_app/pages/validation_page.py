...
def main():
    ...
    if not controle_id:
        st.warning(
            "Aucun contrôle sélectionné. Veuillez choisir un groupe et une année dans la barre latérale."
        )
        return
    ...
    with Session(engine) as session:
        ...
        with col_table:
            ...
            if poste_id is None:  # Option "Tout" sélectionnée
                ...
                for poste in postes:
                    factures.extend(
                        session.exec(select(Facture).where(Facture.poste_id == poste.id)).all()
                    )
                poste_actuel = None
            else:
        ...
        with col_pdf:
            ...
            with Session(engine) as session:
                ...
                if factures_avec_pdf:  # Si des PDFs existent
                    factures_menu = {
                        f"Facture {f['Numéro']} - {f['Montant']:.2f}€": f["ID"]
                        for f in factures_avec_pdf
                    }
                    ...
                    with col_nav1:
                        if st.button("⬅️ Précédent"):
                            if st.session_state.selected_facture_id:
                                current_index = list(factures_menu.values()).index(
                                    st.session_state.selected_facture_id
                                )
                                if current_index > 0:
                                    st.session_state.selected_facture_id = list(
                                        factures_menu.values()
                                    )[current_index - 1]

                    with col_nav2:
                        try:
                            default_index = (
                                list(factures_menu.values()).index(
                                    st.session_state.selected_facture_id
                                )
        ...
        if poste_id is None:  # Option "Tout" sélectionnée
            ...
            nouvelle_remarque = st.text_area(
                "Éditer la remarque globale", value=remarque_globale, height=400
            )
            ...
            if st.button("Enregistrer la remarque globale"):
                with Session(engine) as session:
                    ...
                    if controle_maj is not None:
                        ...
                        st.error(
                            "Erreur : Contrôle introuvable lors de la mise à jour de la remarque globale."
                        )
