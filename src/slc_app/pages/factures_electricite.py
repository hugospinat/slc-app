...
def import_models():
    ...
    from slc_app.models import ControleCharges, Facture, FactureElectricite, Groupe, engine

    return ControleCharges, Facture, FactureElectricite, Groupe, engine


# Import avec cache
ControleCharges, Facture, FactureElectricite, Groupe, engine = import_models()
    ...
    with Session(engine) as session:
        ...
        controle = session.exec(
            select(ControleCharges).where(
                ControleCharges.groupe_id == groupe_id, ControleCharges.annee == annee
            )
        ).first()
            ...
            factures_data.append(
                {
                    ...
                    "Fournisseur": (
                        facture.fournisseur.nom if facture.fournisseur else "Non détecté"
                    ),
                    "Statut": facture.statut,
                    "Commentaire": facture.commentaire_contestation or "",
                    "Index Début": (
                        details.index_debut if details and details.index_debut is not None else 0.0
                    ),
                    "Index Fin": (
                        details.index_fin if details and details.index_fin is not None else 0.0
                    ),
                    "Date Début": (
                        details.date_debut.date()
                        if details and details.date_debut
                        else datetime.now().date()
                    ),
                    "Date Fin": (
                        details.date_fin.date()
                        if details and details.date_fin
                        else datetime.now().date()
                    ),
                }
            )
            ...
            selected_rows = st.data_editor(
                ...
                column_config={
                    ...
                    "Référence": st.column_config.TextColumn(
                        "Référence", disabled=True, width="medium"
                    ),
                    "Fournisseur": st.column_config.TextColumn(
                        "Fournisseur", disabled=True, width="medium"
                    ),
                    "Statut": st.column_config.SelectboxColumn(
                        "Statut",
                        options=["en_attente", "validee", "contestee"],
                        required=True,
                        width="small",
                    ),
                    "Commentaire": st.column_config.TextColumn(
                        "Commentaire", disabled=False, width="large"
                    ),
                    ...
                    "Date Début": st.column_config.DateColumn(
                        "Date Début", disabled=False, width="medium"
                    ),
                    "Date Fin": st.column_config.DateColumn(
                        "Date Fin", disabled=False, width="medium"
                    ),
                },
                column_order=[
                    "Numéro",
                            ...
                            try:
                                ...
                                if facture_db:
                                    ...
                                    details_db = update_session.exec(
                                        select(FactureElectricite).where(
                                            FactureElectricite.facture_id == row["ID"]
                                        )
                                    ).first()
                                    ...
                                    if details_db:
                                        ...
                                        details_db.date_debut = datetime.combine(
                                            row["Date Début"], datetime.min.time()
                                        )
                                        details_db.date_fin = datetime.combine(
                                            row["Date Fin"], datetime.min.time()
                                        )
                                ...
                                st.error(
                                    f"❌ Erreur lors de la mise à jour de la facture {row['Numéro']}: {e}"
                                )
